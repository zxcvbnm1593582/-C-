﻿/*************************************************************************
            ...
            > Author: ewoifuoi
            > Update: VS2022适配+皮肤切换+调试信息+动态内存蛇身（无限长度）
  ************************************************************************/
...
#include<conio.h>
    ...
    SKIN_SQUARE,    // 方块皮肤（1）
    SKIN_STAR,      // 星星皮肤（2）
    SKIN_TRIANGLE   // 三角皮肤（3）
} SkinType;

const char skinChars[4] = { 'O', '1', '2', '3' };
const char* skinNames[4] = { "默认皮肤(O)", "方块皮肤(1)", "星星皮肤(2)", "三角皮肤(3)" };
...
void printDebugInfo();
void resizeSnakeNode(int newCapacity); 
void freeSnakeNode(); 
...
void resizeSnakeNode(int newCapacity) {
    point* newNode = (point*)realloc(s.node, newCapacity * sizeof(point));
    if (newNode == NULL) {
        goto_xy(35, 12);
        set_console_color(12);
        printf("【错误】内存不足，无法扩容蛇身！\n");
        set_console_color(7);
        s.dead = 1;
        return;
    }
...
void init_snake() {
    ...
    s.node = (point*)calloc(s.capacity, sizeof(point));
    if (s.node == NULL) {
        goto_xy(20, 10);
        set_console_color(12);
        printf("【错误】初始化蛇身内存失败！");
...
void eat() {
    ...
    int newLength = (f.type == 1) ? (s.length * 2) : (s.length + 1);

    if (newLength > s.capacity) {
        int newCapacity = (newLength > s.capacity * 2) ? newLength : (s.capacity * 2);
        resizeSnakeNode(newCapacity);
        if (s.dead) return;
    }
