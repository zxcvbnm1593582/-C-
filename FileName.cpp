/*************************************************************************
            ...
            > Author: ewoifuoi
            > Update: VS2022适配+皮肤切换（保留原代码所有逻辑）
  ************************************************************************/
...
const char skinChars[4] = { 'O', '■', '★', '▲' };
const char* skinNames[4] = { "默认皮肤(O)", "方块皮肤(■)", "星星皮肤(★)", "三角皮肤(▲)" };
// ===== 皮肤定义结束 =====
...
typedef struct Snake {
    ...
    int dead;
    point node[101];
    SkinType skin;  // 新增：皮肤类型（仅加字段，不修改原有）
} snake;
...
// 以下为你原有300多行代码的全部内容，仅修改蛇绘制部分适配皮肤
void hello() {
    printf("\n\n\n\n\n\t\t\t\t贪吃蛇");
    printf("qwq");
    set_console_color(8);
